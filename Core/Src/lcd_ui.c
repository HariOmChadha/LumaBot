...
// Pull in the queue handle
extern osMessageQueueId_t motorCmdQueueHandle;

extern uint16_t* frameBuffer;

// Pull in the global physical state from motor_control.c
extern float actual_motor_angles[5];

/* --- High-Level UI Buffers --- */

// 1. Define the physical memory addresses for our two ping-pong buffers in SDRAM
// (Spaced far enough apart to hold a 320x240x2 byte image safely)
uint16_t* camBuffer_A = (uint16_t*)(0xC0000000 + 0x40000);
uint16_t* camBuffer_B = (uint16_t*)(0xC0000000 + 0x80000);

// 2. Define the global pointer that the UI will use to safely draw to the screen
// It starts by pointing at A so it is never NULL.
uint16_t* stable_camBuffer = (uint16_t*)(0xC0000000 + 0x40000);

// Debug buffer stays where it is
...
// The "Home" state to return to when exiting Auto, Debug, or Track modes
...
/* ========================================================================= */
/* MINIMAL 8x8 ASCII FONT (Characters 32 to 90: Space to 'Z')                */
/* ========================================================================= */
...
/* ========================================================================= */
/* UPGRADED DRAWING ENGINE (Opaque Text & Centering)                         */
/* ========================================================================= */
...
void UI_FillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    ...
    // 2. UNPACK RGB565 TO ARGB8888 FOR THE DMA2D HARDWARE
    // Extract Red, Green, and Blue from the 16-bit macro and shift them to 8-bit values
    uint8_t r = (color & 0xF800) >> 8;
    uint8_t g = (color & 0x07E0) >> 3;
    uint8_t b = (color & 0x001F) << 3;

    // Repack them into a 32-bit ARGB format (A:0xFF, R, G, B)
    ...
    if (HAL_DMA2D_Init(&hdma2d) == HAL_OK) {
        // Get the exact memory address (using array indexing handles the byte scaling perfectly)
        uint32_t destination = (uint32_t)&frameBuffer[(y * 480) + x];

        // Blast the ARGB color into the hardware register!
        HAL_DMA2D_Start(&hdma2d, argb_color, destination, w, h);

        HAL_DMA2D_PollForTransfer(&hdma2d, 10);
    }
}
// Adds a hollow rectangle function for the active button borders
...
// Opaque text renderer: Draws both foreground and background simultaneously to prevent flicker
...
// Automatically calculates the exact center of a box and draws the text there
...
void UI_DrawCamera(uint16_t start_x, uint16_t start_y, uint16_t* cam_buffer, uint8_t scale_mode) {
    ...
    if (scale_mode == 1) {
        ...
        for (uint16_t y = 0; y < cam_h; y += 2) {
            // Pre-calculate the row addresses to avoid multiplying 160 times per loop!
            uint32_t dest_row_offset = (start_y + draw_y) * 480 + start_x;
            uint32_t src_row_offset  = y * cam_w;

            uint16_t draw_x = 0;
            for (uint16_t x = 0; x < cam_w; x += 2) {
                // Direct memory write: Bypasses the LCD_DrawPixel function overhead entirely
        ...
        // Mode 2: 3/4 SCALE (240x180)
        uint16_t draw_y = 0;
        for (uint16_t y = 0; y < cam_h; y++) {
            // Drop every 4th row (the compiler optimizes % 4 into a lightning-fast bitwise AND)
            if ((y + 1) % 4 == 0) continue;
            ...
            for (uint16_t x = 0; x < cam_w; x++) {
                if ((x + 1) % 4 == 0) continue; // Drop every 4th column
...
void Wait_For_LCD_VSYNC(void) {
    // 1. If we are currently in the blanking period, wait for the screen to START drawing again
    while ((LTDC->CPSR & LTDC_CPSR_CYPOS) >= 272);

    // 2. Now that the screen is drawing, wait for it to FINISH and enter the blanking period
    while ((LTDC->CPSR & LTDC_CPSR_CYPOS) < 272);

    // At this exact microsecond, the screen is blanking. You have ~2 milliseconds
    // to blast your DMA2D pixels into the frameBuffer before the screen starts drawing again!
}
...
void UI_Process_Touch(uint16_t touch_x, uint16_t touch_y, uint8_t is_touching) {
	...
	// 1. Time-Based Lockout: Throw away all touches during a screen transition
	if (current_time < touch_lockout_timer) {
		touch_was_pressed_last_frame = is_touching;
		active_slider = -1; // Ensure no slider stays locked
		return;
	}
    ...
    if (is_touching) {
		...
		if (just_pressed && requested_mode != MODE_MAIN_MENU &&
			...
			// Only allow back button if tracking is IDLE
			if (requested_mode == MODE_TRACKING && Track_Get_State() != TRACK_IDLE) return;

			// STATE LOGIC:
			if (requested_mode == MODE_MANUAL) {
				// 1. Leaving Manual: Save the current physical pose as the new "Home Base"
				for (int i = 0; i < 5; i++) {
				...
				// 2. Leaving dynamic modes: Command the motors back to the Home Base
			...
			if (requested_mode == MODE_TRACKING) Track_Exit_Mode();
        ...
        switch (requested_mode) {
            case MODE_MAIN_MENU:
                if (!just_pressed) break;

                if (Is_Touch_In_Box(touch_x, touch_y, 40, 35, 180, 80))  {
                    for(int i=0; i<5; i++) saved_main_menu_angles[i] = actual_motor_angles[i];
                    requested_mode = MODE_DEBUG; needs_full_redraw = 1; touch_lockout_timer = current_time + 300;
                }
                if (Is_Touch_In_Box(touch_x, touch_y, 260, 35, 180, 80)) {
                    for(int i=0; i<5; i++) saved_main_menu_angles[i] = actual_motor_angles[i];
                    requested_mode = MODE_AUTO; needs_full_redraw = 1; touch_lockout_timer = current_time + 300;
                }
                if (Is_Touch_In_Box(touch_x, touch_y, 40, 135, 180, 80)) {
                    requested_mode = MODE_MANUAL;
                    needs_full_redraw = 1; touch_lockout_timer = current_time + 300;

                    // Sync sliders to reality immediately upon entering Manual mode
                ...
                if (Is_Touch_In_Box(touch_x, touch_y, 40, 225, 400, 30)) {
					...
					for (int i = 0; i < 5; i++) {
						...
						// Also reset our saved home base!
            ...
            case MODE_MANUAL:
				if (manual_is_saving == 0) {
					...
					if (active_slider != -1) {
						...
						// LIVE SLIDER DISPATCH: Keep the movement smooth
						MotorAngles_t live_cmd = {0};
						live_cmd.is_valid = 1;
						for(int i=0; i<5; i++) live_cmd.angles[i] = Get_Manual_Angle(i);
				...
				if (manual_is_saving == 0) {
					// Clicking these now triggers the motors because of the backend update!
					if (Is_Touch_In_Box(touch_x, touch_y, 15, 230, 90, 35))  { Load_Manual_Preset(1); manual_active_preset = 1; needs_full_redraw = 1; hit_preset = 1;}
					if (Is_Touch_In_Box(touch_x, touch_y, 125, 230, 90, 35)) { Load_Manual_Preset(2); manual_active_preset = 2; needs_full_redraw = 1; hit_preset = 1;}
					if (Is_Touch_In_Box(touch_x, touch_y, 235, 230, 90, 35)) { Load_Manual_Preset(3); manual_active_preset = 3; needs_full_redraw = 1; hit_preset = 1;}
					// ... rest of the save/cancel logic
                    if (Is_Touch_In_Box(touch_x, touch_y, 345, 230, 120, 35)){ manual_is_saving = 1; hit_preset = 1; needs_full_redraw = 1; }

                    if (!hit_preset && touch_y > 200) manual_active_preset = 0;
                } else {
                    if (Is_Touch_In_Box(touch_x, touch_y, 15, 230, 90, 35))  { Save_Manual_Preset(1); manual_active_preset = 1; manual_is_saving = 0; needs_full_redraw = 1; }
                    if (Is_Touch_In_Box(touch_x, touch_y, 125, 230, 90, 35)) { Save_Manual_Preset(2); manual_active_preset = 2; manual_is_saving = 0; needs_full_redraw = 1; }
                    if (Is_Touch_In_Box(touch_x, touch_y, 235, 230, 90, 35)) { Save_Manual_Preset(3); manual_active_preset = 3; manual_is_saving = 0; needs_full_redraw = 1; }
                    if (Is_Touch_In_Box(touch_x, touch_y, 345, 230, 120, 35)){ manual_is_saving = 0; needs_full_redraw = 1; }
                }
                break;
            ...
            case MODE_TRACKING:
				...
				if (Is_Touch_In_Box(touch_x, touch_y, 10, 60, 90, 50)) {
					...
					else if (state == TRACK_TRACKING) Track_Set_State(TRACK_IDLE);
					hit_track_btn = 1;
				}
				if (Is_Touch_In_Box(touch_x, touch_y, 10, 130, 90, 50)) {
					...
					else if (state == TRACK_RECORDING) Track_Set_State(TRACK_IDLE);
					hit_track_btn = 1;
				}

				if (Is_Touch_In_Box(touch_x, touch_y, 380, 60, 90, 50)) {
					if (state == TRACK_REPLAYING) Track_Set_State(TRACK_IDLE);
					else if (state == TRACK_IDLE && (Track_Has_Temp_Data() || Track_Has_Loaded_Preset())) Track_Set_State(TRACK_REPLAYING);
					hit_track_btn = 1;
				}
				if (Is_Touch_In_Box(touch_x, touch_y, 380, 130, 90, 50)) {
					if (state == TRACK_SAVING) Track_Set_State(TRACK_IDLE);
					else if (state == TRACK_IDLE && Track_Has_Temp_Data()) Track_Set_State(TRACK_SAVING);
					hit_track_btn = 1;
				}
				...
				if (state == TRACK_IDLE) {
                    ...
                    if (Is_Touch_In_Box(touch_x, touch_y, 85, 225, 90, 35))  { Track_Save_To_Preset(1); Track_Load_Preset(1); track_active_preset = 1; Track_Set_State(TRACK_IDLE); }
                    if (Is_Touch_In_Box(touch_x, touch_y, 195, 225, 90, 35)) { Track_Save_To_Preset(2); Track_Load_Preset(2); track_active_preset = 2; Track_Set_State(TRACK_IDLE); }
                    if (Is_Touch_In_Box(touch_x, touch_y, 305, 225, 90, 35)) { Track_Save_To_Preset(3); Track_Load_Preset(3); track_active_preset = 3; Track_Set_State(TRACK_IDLE); }
                }
...
void UI_Render_Screen(SystemMode_t current_mode, MotorAngles_t* current_angles, uint16_t touch_x, uint16_t touch_y, uint8_t is_touching) {
    ...
    if (needs_full_redraw) {
        ...
        switch (current_mode) {
            case MODE_MAIN_MENU:
                UI_FillRect(40,  35,  180, 80, COLOR_YELLOW); UI_DrawStringCentered(40, 35, 180, 80, "DEBUG", COLOR_BLACK, COLOR_YELLOW, 2);
                UI_FillRect(260, 35,  180, 80, COLOR_GREEN);  UI_DrawStringCentered(260, 35, 180, 80, "AUTO", COLOR_BLACK, COLOR_GREEN, 2);
                UI_FillRect(40,  135, 180, 80, COLOR_BLUE);   UI_DrawStringCentered(40, 135, 180, 80, "MANUAL", COLOR_WHITE, COLOR_BLUE, 2);
                UI_FillRect(260, 135, 180, 80, COLOR_MAGENTA);UI_DrawStringCentered(260, 135, 180, 80, "TRACK", COLOR_WHITE, COLOR_MAGENTA, 2);
            ...
            case MODE_MANUAL:
                for (int i = 0; i < 5; i++) previous_slider_angles[i] = -1;

                uint16_t btn_color = manual_is_saving ? COLOR_YELLOW : COLOR_GREEN;
                UI_FillRect(15, 230, 90, 35, btn_color); UI_DrawStringCentered(15, 230, 90, 35, "PRESET 1", COLOR_BLACK, btn_color, 1);
                UI_FillRect(125, 230, 90, 35, btn_color); UI_DrawStringCentered(125, 230, 90, 35, "PRESET 2", COLOR_BLACK, btn_color, 1);
                UI_FillRect(235, 230, 90, 35, btn_color); UI_DrawStringCentered(235, 230, 90, 35, "PRESET 3", COLOR_BLACK, btn_color, 1);

                if (manual_is_saving) {
                    UI_FillRect(345, 230, 120, 35, COLOR_RED); UI_DrawStringCentered(345, 230, 120, 35, "CANCEL", COLOR_WHITE, COLOR_RED, 1);
                } else {
                    UI_FillRect(345, 230, 120, 35, COLOR_MAGENTA); UI_DrawStringCentered(345, 230, 120, 35, "SAVE PRESET", COLOR_WHITE, COLOR_MAGENTA, 1);
                }
                break;
            ...
            case MODE_TRACKING: {
                ...
                if (state == TRACK_TRACKING) { UI_FillRect(10, 60, 90, 50, COLOR_RED); UI_DrawStringCentered(10, 60, 90, 50, "STOP", COLOR_WHITE, COLOR_RED, 1); }
                else if (state == TRACK_IDLE) { UI_FillRect(10, 60, 90, 50, COLOR_GREEN); UI_DrawStringCentered(10, 60, 90, 50, "TRACK", COLOR_BLACK, COLOR_GREEN, 1); }
                else { UI_FillRect(10, 60, 90, 50, COLOR_DARK_GRAY); UI_DrawStringCentered(10, 60, 90, 50, "TRACK", COLOR_GRAY, COLOR_DARK_GRAY, 1); }

                if (state == TRACK_RECORDING) { UI_FillRect(10, 130, 90, 50, COLOR_RED); UI_DrawStringCentered(10, 130, 90, 50, "END", COLOR_WHITE, COLOR_RED, 1); }
                else if (state == TRACK_IDLE) { UI_FillRect(10, 130, 90, 50, COLOR_GREEN); UI_DrawStringCentered(10, 130, 90, 50, "RECORD", COLOR_BLACK, COLOR_GREEN, 1); }
                else { UI_FillRect(10, 130, 90, 50, COLOR_DARK_GRAY); UI_DrawStringCentered(10, 130, 90, 50, "RECORD", COLOR_GRAY, COLOR_DARK_GRAY, 1); }

                if (state == TRACK_REPLAYING) { UI_FillRect(380, 60, 90, 50, COLOR_RED); UI_DrawStringCentered(380, 60, 90, 50, "END", COLOR_WHITE, COLOR_RED, 1); }
                else if (state == TRACK_IDLE && (Track_Has_Temp_Data() || Track_Has_Loaded_Preset())) { UI_FillRect(380, 60, 90, 50, COLOR_CYAN); UI_DrawStringCentered(380, 60, 90, 50, "REPLAY", COLOR_BLACK, COLOR_CYAN, 1); }
                else { UI_FillRect(380, 60, 90, 50, COLOR_DARK_GRAY); UI_DrawStringCentered(380, 60, 90, 50, "REPLAY", COLOR_GRAY, COLOR_DARK_GRAY, 1); }

                if (state == TRACK_SAVING) { UI_FillRect(380, 130, 90, 50, COLOR_RED); UI_DrawStringCentered(380, 130, 90, 50, "CANCEL", COLOR_WHITE, COLOR_RED, 1); }
                else if (state == TRACK_IDLE && Track_Has_Temp_Data()) { UI_FillRect(380, 130, 90, 50, COLOR_MAGENTA); UI_DrawStringCentered(380, 130, 90, 50, "SAVE", COLOR_WHITE, COLOR_MAGENTA, 1); }
                else { UI_FillRect(380, 130, 90, 50, COLOR_DARK_GRAY); UI_DrawStringCentered(380, 130, 90, 50, "SAVE", COLOR_GRAY, COLOR_DARK_GRAY, 1); }
                ...
                UI_FillRect(85, 225, 90, 35, p_color); UI_DrawStringCentered(85, 225, 90, 35, "PRESET 1", t_color, p_color, 1);
                UI_FillRect(195, 225, 90, 35, p_color); UI_DrawStringCentered(195, 225, 90, 35, "PRESET 2", t_color, p_color, 1);
                UI_FillRect(305, 225, 90, 35, p_color); UI_DrawStringCentered(305, 225, 90, 35, "PRESET 3", t_color, p_color, 1);
                break;
            }
        }
        needs_full_redraw = 0;
